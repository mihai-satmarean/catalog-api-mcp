import { getProductData } from './src/lib/providers/xd-connects/client';
import { db, productProviders } from './src/db';
...
async function importXDConnectsProducts() {
  try {
    console.log('Fetching XD Connects product data...');
    ...
    for (const product of data) {
      if (!product.ItemCode) {
        console.warn('Skipping product without ItemCode');
        continue;
      }
    ...
    console.log(`\n✅ Successfully imported ${savedProducts.length} products`);
    if (errors.length > 0) {
